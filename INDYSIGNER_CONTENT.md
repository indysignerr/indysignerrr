# Indysigner — Contenu complet du site

Ce document contient tout le contenu éditorial à intégrer dans le site.

---

## IDENTITÉ

- **Nom commercial** : Indysigner
- **Dirigeant** : Indy FRANCOIS
- **Forme juridique** : Entreprise individuelle — Micro-entreprise
- **Adresse** : 222 chemin de la Brague, 06410 Biot, France
- **Téléphone** : 07 69 76 20 76
- **Email** : contact@indysigner.fr
- **SIRET** : 10304455800015
- **Code APE / NAF** : 6201Z — Programmation informatique
- **TVA** : Non applicable (article 293 B du CGI)
- **Cible** : B2B uniquement (professionnels)
- **Site** : indysigner.fr
- **Hébergeur** : Cloudflare, Inc.
- **Instagram** : @_indysigner — https://www.instagram.com/_indysigner/
- **LinkedIn** : Indy François — https://www.linkedin.com/in/indy-fran%C3%A7ois-37a451284/

---

## BASELINE & MANIFESTO

**Baseline** : Votre site en 10 jours chrono.

**Manifesto** :
> On conçoit des sites qui marquent les esprits. Design unique. Livraison express. Prix juste.

**Meta description** : Agence web indépendante française. Sites vitrines premium, livrés en 7 jours. Propriété totale du client sur son site.

---

## SERVICES (3 cards)

### 01. Site Vitrine Sur-Mesure
Design premium sur-mesure, rédaction SEO locale, intégration complète, livré en 7 jours.

### 02. Motion Design & Animations
Scroll narratif, transitions cinématiques, micro-interactions qui transforment un site en expérience.

### 03. Maintenance & Évolution
Modifications illimitées, corrections, optimisations continues. Tu ne touches à rien, ton site vit.

---

## PROCESS (4 étapes)

**01. Briefing**
Brief gratuit sous 48h. On cadre ton projet, tes objectifs, ta cible.

**02. Maquette**
Maquette visuelle avant tout engagement. Si tu aimes pas, tu pars.

**03. Développement**
7 jours ouvrés de la validation à la mise en ligne. Suivi temps réel.

**04. Livraison & Propriété**
Transfert complet du code, domaine et admin. Le site t'appartient à vie.

---

## VALEURS (page À propos)

**Rapidité** — 7 jours de la signature au lancement. On ne traîne pas.

**Transparence** — Prix clairs, processus limpide, communication directe. Zéro mauvaise surprise.

**Innovation** — On utilise les meilleures technologies et l'IA pour aller plus vite sans sacrifier la qualité.

**Accessibilité** — Un site premium ne devrait pas coûter 5 000€. On rend la qualité accessible.

**Pull quote** : "Pas un template. Un site qui te ressemble."

---

## STATS

| Métrique | Valeur |
|----------|--------|
| Sites livrés | 30+ |
| Satisfaction clients | 100% |
| Délai moyen | 7 jours |
| Temps de réponse | 24h |

---

## PORTFOLIO — 6 PROJETS

### 1. Gaoson's Barber Shop
- **Catégorie** : Site Vitrine
- **Année** : 2025 · **Durée** : 10 jours
- **URL** : https://gaoson-barbershop.fr
- **Tags** : Next.js, Tailwind, Booksy, SEO Local

**Contexte**
Barbershop premium à Biot, Côte d'Azur. Identité streetwear + raffinement, typographies gothiques, univers sneakers parallèle.

**Problème**
Aucune présence en ligne à la hauteur de l'identité. Clients découvraient uniquement par bouche-à-oreille ou Instagram. Perte en crédibilité face aux concurrents mieux référencés.

**Solution**
Site vitrine premium capturant l'atmosphère. Design full black avec typographie gothique sur-mesure, hero immersif, section prestations épurée (Coupe 20€, Coupe & Barbe 25€) avec CTA Booksy, section sneakers avec renvoi Instagram.

**Résultats**
- Top 3 Google "barbershop Biot"
- 60% ↑ réservations Booksy
- 2m15s temps moyen de visite
- 10j délai livraison

**Témoignage** (Gaoson, fondateur)
> "Le site ressemble exactement à ce qu'on est. Les gens arrivent au salon et nous disent que le site leur avait déjà donné envie de venir. Indy a capté notre univers dès le premier échange."

---

### 2. KRNL Studio
- **Catégorie** : Sur-Mesure
- **Année** : 2024 · **Durée** : 3 semaines
- **Tags** : Next.js, Stripe, Prisma, GSAP

**Contexte**
Marque de streetwear éco-responsable.

**Problème**
Croissance limitée par Instagram, pas d'e-commerce propre.

**Solution**
Site immersif full-page avec animations GSAP scroll, checkout Stripe, inventaire Prisma, pages produits avec traçabilité matières.

**Résultats**
- 4,2% conversion
- Panier moyen 127€
- 340+ ventes premier mois
- 4m32s temps moyen

**Témoignage**
> "Une expérience immersive qui reflète parfaitement notre univers."

---

### 3. WakeApp
- **Catégorie** : Site Vitrine
- **Année** : 2026 · **Durée** : 12 jours
- **URL** : https://the-wakeapp.com
- **Tags** : Next.js, Framer Motion, App Store, Conversion

**Contexte**
Application mobile de productivité et discipline personnelle.

**Problème**
Pas de landing page → invisible ; utilisateurs ne comprenaient pas l'app.

**Solution**
Landing page immersive cohérente avec l'app, hero iPhone mockup, cards modules (To-Do, Calendar, Notes), "CEO Mode" mis en avant, CTA App Store.

**Résultats**
- 18% conversion visiteur → téléchargement
- 350% ↑ téléchargements
- 1m50s temps moyen

**Témoignage** (CEO WakeApp)
> "La landing page explique tout en 30 secondes et les gens téléchargent."

---

### 4. Côte & Saveurs
- **Catégorie** : Sur-Mesure
- **Année** : 2025 · **Durée** : 4 semaines
- **Tags** : Next.js, Stripe, Google Calendar API

**Contexte**
Traiteur méditerranéen haut de gamme.

**Problème**
Commandes manuelles par téléphone/email, erreurs fréquentes, pas de présence en ligne au niveau du positionnement premium.

**Solution**
Plateforme sur-mesure avec catalogue visuel, système de devis interactif, sélection créneaux via Google Calendar API, acompte Stripe, dashboard admin.

**Résultats**
- 45+ commandes en ligne / mois
- 0 erreur de commande
- 35% ↑ chiffre d'affaires
- 98% satisfaction

**Témoignage**
> "Les clients commandent en ligne sans même nous appeler — et les commandes sont parfaites du premier coup."

---

### 5. Atelier Noma
- **Catégorie** : Refonte
- **Année** : 2025 · **Durée** : 3 semaines
- **Tags** : Next.js, GSAP, Three.js, Framer Motion

**Contexte**
Studio d'architecture d'intérieur.

**Problème**
Site de 2017 — statique, non-responsive, ne reflétait pas la dimension artistique.

**Solution**
Refonte complète avec narration éditoriale, galerie GSAP interactive avec transitions fluides, zoom haute résolution, scroll reveals, parallax, formulaire de qualification de projets.

**Résultats**
- 3× temps sur le site
- 12+ demandes qualifiées / mois
- 45% ↓ taux de rebond
- 98/100 Lighthouse

**Témoignage**
> "Notre site reflète enfin l'ambition de notre studio."

---

### 6. GreenPulse
- **Catégorie** : Site Vitrine
- **Année** : 2025 · **Durée** : 2 semaines
- **URL** : https://greenpulse.io
- **Tags** : Next.js, D3.js, GSAP, Framer Motion

**Contexte**
Startup cleantech en levée de fonds Série A de 3M€.

**Problème**
Site WordPress générique, impossible de démontrer le produit, manque de crédibilité auprès des investisseurs.

**Solution**
Site tech-forward avec animations de données temps réel (D3.js), démo interactive dashboard SaaS, visualisations impact CO₂, téléchargement deck investisseur.

**Résultats**
- 4m12s temps moyen investisseur
- Série A 3M€ closée en 6 semaines
- 99/100 PageSpeed
- 28+ leads qualifiés / mois

**Témoignage**
> "Plusieurs investisseurs nous ont dit que notre site était le meilleur qu'ils avaient vu. La levée s'est clôturée en 6 semaines."

---

## PRICING — PAGE `/tarifs`

### Header

**Surtitre** : TARIFS TRANSPARENTS
**H1** : Deux formules. Un site qui t'appartient. Zéro surprise.
**Sous-titre** : Choisis selon que tu préfères tout maîtriser toi-même, ou qu'Indysigner s'occupe de tout pour toi. Pas de piège, pas de clauses cachées, pas d'abonnement qui te retient.

### FORMULE 1 — AUTONOMIE
- **Badge** : LE PLUS VENDU
- **Accroche** : Tu payes une fois. C'est à toi. Pour toujours.
- **Prix** : 490€ · une seule fois · tout compris
- **Ligne contexte** : Puis 0€. À vie.

**Inclus** :
- Création site vitrine sur-mesure
- Design premium + animations au scroll
- Rédaction complète (FR, SEO local)
- Intégration photos fournies ou sourcées
- Version mobile optimisée
- Interface admin simple
- Setup technique complet (domaine, hébergement, admin)
- Formation 1h (visio ou présentiel)
- Manuel d'utilisation PDF
- Support technique 30 jours
- Hébergement gratuit à vie

**Après 30 jours** : grille tarifaire claire à partir de 35€.

**CTA** : Démarrer mon projet →
**Micro-copy** : Devis sous 24h. Maquette gratuite avant tout engagement.

### FORMULE 2 — SÉRÉNITÉ
- **Badge** : LE PLUS TRANQUILLE
- **Accroche** : Tu ne touches à rien. Jamais. Je m'occupe de tout.
- **Prix** : 250€ + 200€/an (ou 20€/mois)
- **Ligne contexte** : + 10€/mois si tu veux que je gère aussi ton domaine

**Inclus** :
- Tout Autonomie (sans la formation)
- Modifications illimitées (textes, photos, horaires)
- Ajout de pages à la demande
- Refonte de sections
- Widgets tiers (réservation, carte, formulaire)
- SEO continu
- Corrections bugs et mises à jour
- Support sous 48h ouvrées
- Conseils stratégiques

**Petit texte** : Annuel (200€) = 2 mois offerts. Résiliable à tout moment avec 30 jours préavis. Ton site reste à toi.

**CTA** : Démarrer mon projet →

---

## GRILLE TARIFAIRE COMPLÈTE

### Formules principales

| Formule | Prix | Périodicité |
|---------|------|-------------|
| Autonomie | 490€ | One-shot |
| Sérénité mensuel | 250€ + 20€/mois | Récurrent |
| Sérénité annuel | 250€ + 200€/an | 2 mois offerts |
| Option domaine géré (Sérénité) | +10€/mois ou +100€/an | Additionnel |

### Interventions ponctuelles Autonomie (après 30j)

| Prestation | Tarif |
|------------|-------|
| Intervention simple | 35€ |
| Intervention complexe | 80€ |
| Refonte section | 100€ |
| Ajout page | 100€ |
| Formation 1h | 60€ |
| Widget tiers | 60€ |
| Récupération site cassé | 180€ |

### Packs Autonomie

| Pack | Prix | Économie |
|------|------|----------|
| 3 interventions simples | 90€ | 15€ |
| 5 interventions simples | 140€ | 35€ |

Valables 12 mois, non remboursables.

### Prestations complémentaires (2 formules)

| Prestation | Tarif |
|------------|-------|
| Pack photos pro (shooting 2h) | 250€ |
| Pack visuels IA (6 illustrations) | 120€ |
| Article SEO 800 mots | 80€ |
| Logo simple | 150€ |
| Setup Google My Business | 80€ |
| Connexion formulaire CRM | 60€ |

---

## COMPARATIF ANTI-WIX

**Titre** : Sur Wix, tu loues une cage dorée. Chez Indysigner, tu construis ta maison.

| Critère | Wix/Squarespace | Autonomie | Sérénité annuelle |
|---------|-----------------|-----------|-------------------|
| Coût 1ère année | 192€ | 490€ | 450€ |
| Coût année 2 | 192€ | 0€ | 200€ |
| Coût total 3 ans | 576€ | 490€ | 850€ |
| Design sur-mesure | ❌ Templates | ✅ | ✅ |
| Propriété du site | ❌ | ✅ | ✅ |
| Hébergement | Loyer à vie | Gratuit à vie | Inclus |
| Si tu arrêtes | Site supprimé | Site reste à toi | Site reste à toi |
| Qualité design | Moyenne | Premium | Premium |

---

## FAQ PRICING (10 questions)

**Q1 — Est-ce que je peux vraiment modifier mon site moi-même sans connaissances techniques ?**
Oui, complètement. L'interface d'administration ressemble à un formulaire : tu cliques sur "Horaires", tu écris tes nouveaux horaires, tu cliques "Publier". 2 minutes plus tard, ton site est à jour. Je te forme pendant 1h pour que tu sois à l'aise. Personne n'a jamais eu besoin d'une deuxième formation.

**Q2 — Qu'est-ce qui se passe si j'arrête de payer la maintenance (formule Sérénité) ?**
Ton site reste à toi. Tu récupères tes accès complets (domaine, hébergement, interface admin), tu continues à gérer ton site seul. Aucune coupure, aucune perte. La différence avec Wix : tu ne redémarres pas à zéro, tu continues avec TON site existant.

**Q3 — Pourquoi c'est moins cher que les autres agences ?**
Parce que j'utilise des outils d'IA avancés pour accélérer la création. Ce qui prenait 3 semaines prend 3 jours. Je répercute ce gain sur le prix. La qualité ne change pas : chaque site est pensé sur-mesure selon ton secteur, ta cible, ton positionnement.

**Q4 — Je peux commencer en Autonomie et passer en Sérénité plus tard ?**
Absolument. Tu payes 490€ pour Autonomie, et si dans 6 mois tu en as marre de modifier toi-même, tu bascules en Sérénité. Tu payes alors uniquement les 200€/an (pas de nouveaux frais de création).

**Q5 — Et si je veux partir chez une autre agence ou reprendre la main ?**
Tu pars avec tout. Le code de ton site, ton nom de domaine, ton interface d'administration. Aucune prise d'otage. C'est ma promesse dès le premier jour : propriété totale du client sur son site.

**Q6 — Combien de temps entre la commande et la mise en ligne ?**
7 jours ouvrés entre la validation du brief et la livraison.
- Jour 1-2 : Design et structure
- Jour 3-4 : Rédaction et intégration contenus
- Jour 5 : Version mobile et optimisation
- Jour 6 : Ajustements selon tes retours
- Jour 7 : Mise en ligne et formation (pour Autonomie)

**Q7 — Est-ce que je peux voir ce que ça va donner avant de payer ?**
Oui. Je propose une maquette gratuite sous 48h après réception de ton brief. Tu vois concrètement à quoi ton site ressemblera, et tu décides ensuite.

**Q8 — Vous faites des e-commerces ou des systèmes de réservation ?**
Je fais des sites vitrines exclusivement. Si tu as besoin de réservation (restaurant, consultation) ou de paiement, j'intègre des outils tiers reconnus (Zenchef, Tableo, Stripe Link, Calendly) directement sur ton site. Pas de développement e-commerce custom.

**Q9 — Ça marche pour quels types de métiers ?**
Restaurateurs, artisans, barbiers, kinésithérapeutes, ostéopathes, avocats, notaires, experts-comptables, coaches, consultants, freelances, associations. Si tu n'es pas sûr, envoie-moi un message : je te dis honnêtement si ton cas entre dans mon offre ou non.

**Q10 — Je paye comment ?**
Par carte bancaire via Stripe, en 1 fois ou 2 fois (50% commande, 50% livraison pour Autonomie). Pour Sérénité mensuelle, prélèvement automatique. Pour Sérénité annuelle, paiement unique annuel. Tu reçois une facture pour ta comptabilité.

---

## BANDEAU RÉASSURANCE (footer pricing)

```
🇫🇷 Freelance basé à Biot
Tu parles à Indy directement, personne d'autre.

⚡ Livraison en 7 jours
Pas de mois d'attente, pas de charges inutiles.

🔒 Propriété garantie
Ton site t'appartient. Clause écrite dans le contrat.
```

---

## CTA FINAUX

**Page tarifs (fin)** :
- H2 : On commence par une maquette gratuite ?
- Sous-titre : Tu remplis le brief en 3 minutes, je te livre une maquette dans les 48h. Tu décides après.
- CTA principal : Remplir le brief →
- CTA secondaire : Ou discutons d'abord : 07 69 76 20 76

**Homepage (fin)** :
- H2 : Prêt à lancer votre projet ?
- Sous-titre : Devis gratuit · Réponse sous 24h · Zéro engagement
- CTA : Démarrer un projet

---

## BLOG (5 articles)

Titres à reprendre (contenu à rédiger ou copier de l'ancien site) :
- Glossaire du web — Termes expliqués simplement
- Next.js en 2025 — Pourquoi ce framework
- SEO local Google Maps — Remonter dans les recherches locales
- Motion design utile ? — Quand l'animation sert
- Quand faire une refonte — Les signes

---

## NAVIGATION

Accueil · Services · Portfolio · Tarifs · Blog · À propos · Contact

---

## FORMULAIRE CONTACT

**Titre** : Parlons de votre projet.
**Sous-titre** : Remplis le formulaire, reçois un devis sous 24h.

**Champs** :
1. Nom complet (text, requis) — placeholder : "Jean Dupont"
2. Email (email, requis) — placeholder : "jean@exemple.fr"
3. Type de projet (select, requis) — Site Vitrine · Site Sur-Mesure · Maintenance · Autre
4. Budget estimé (select) — < 500€ · 500-1000€ · > 1000€ · À discuter
5. Message (textarea, requis) — placeholder : "Décris ton projet, tes objectifs, ton délai..."
6. ☐ J'accepte les CGV et la politique de confidentialité (checkbox requise)

**Colonne droite** :
- Email : contact@indysigner.fr
- Téléphone : 07 69 76 20 76
- Localisation : Biot, France — Remote
- Instagram + LinkedIn
- Footer : "Réponse sous 24h en semaine. Devis gratuit, sans engagement."

**Backend** : API Route `/api/contact` + Resend
- From : `Indysigner <noreply@indysigner.fr>`
- To : `contact@indysigner.fr`
- Env : `RESEND_API_KEY`
