...
function validate(input: unknown): { ok: true; data: Required<Omit<ContactPayload, "delay">> & { delay: string } } | { ok: false; error: string } {
  if (!input || typeof input !== "object") {
    return { ok: false, error: "Payload invalide" };
  }
  const { name, email, type, delay, message, consent } = input as ContactPayload;
  ...
  return {
    ...
    data: {
      ...
      email: email.trim(),
      type: type.trim(),
      delay: (delay ?? "Non précisé").toString(),
...
export async function POST(request: Request) {
  ...
  const { name, email, type, delay, message } = parsed.data;
  const subject = `Nouveau brief — ${escape(name)} (${escape(type)})`;
  const html = `
    <div style="font-family: system-ui, sans-serif; max-width: 600px; color: #0b0d12;">
      ...
      <table style="width:100%; border-collapse: collapse; font-size: 14px;">
        ...
        <tr><td style="padding:6px 0; color:#7a7f88;">Offre</td><td style="padding:6px 0;">${escape(type)}</td></tr>
        <tr><td style="padding:6px 0; color:#7a7f88;">Délai</td><td style="padding:6px 0;">${escape(delay)}</td></tr>
      </table>
